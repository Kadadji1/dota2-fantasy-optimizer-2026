...
import { balancedMainEvent, mainEventTeams, MainEventTeam, PlacementOdds } from "../../data/mainEvent";

type Lang = "en" | "ru" | "es" | "zh";
type ModelKey = "historical" | "tiOnly";
type Team = MainEventTeam;
...
const models: Record<ModelKey, Model> = {
  historical: {
    ...
    odds: balancedMainEvent.odds,
    placements: balancedMainEvent.placements,
    maps: balancedMainEvent.maps,
    picks: [
...
function InteractivePredictionBoard({ language }: { language: Lang }) {
  ...
  return <main className="site-shell predictions-page bracket-predictions-page">
    <section className="schedule-strip" aria-label={t.schedule}><strong>{t.schedule}</strong><div>{openingIds.map((id) => { const match = bracket.find((item) => item.id === id)!; return <span key={id}><time>{formatMatchTime(id, language)}</time><b>{match.teams.filter(Boolean).join(" vs ")}</b></span>; })}{laterIds.map((id) => <span key={id}><time>{formatMatchTime(id, language)}</time><b>{matchNames[language][id]}</b></span>)}</div></section>
