...
const languageButtons: Array<{ key: SiteLanguage; label: string }> = [
  { key: "en", label: "EN" },
  { key: "ru", label: "RU" },
  { key: "es", label: "ES" },
  { key: "zh", label: "中文" }
...
const navigation: Record<SiteLanguage, { fantasy: string; predictions: string; guide: string }> = {
  en: { fantasy: "Fantasy Calculator", predictions: "Main Event", guide: "Guide" },
  ru: { fantasy: "Фэнтези-калькулятор", predictions: "Main Event", guide: "Гайд" },
  es: { fantasy: "Calculadora Fantasy", predictions: "Evento Principal", guide: "Guía" },
  zh: { fantasy: "梦幻计算器", predictions: "主赛事", guide: "指南" }
};
...
export default function SiteHeader({ active }: { active: ActivePage }) {
  ...
  useEffect(() => {
    ...
    if (saved && languageButtons.some((item) => item.key === saved)) setLanguage(saved);
  }, []);
    ...
    <header className="site-master-header">
      <div className="site-master-row">
        ...
        <nav className="site-master-tabs" aria-label="Primary navigation">
          <Link href="/" className={active === "fantasy" ? "active" : ""}>{t.fantasy}</Link>
          <Link href="/predictions" className={active === "predictions" ? "active" : ""}>{t.predictions}</Link>
          <Link href="/guide" className={active === "guide" ? "active" : ""}>{t.guide}</Link>
        </nav>

        <div className="site-master-languages" aria-label="Language switcher">
          {languageButtons.map(({ key, label }) => <button key={key} className={language === key ? "active" : ""} onClick={() => setLanguage(key)}>{label}</button>)}
        </div>
      </div>
    </header>
  );
