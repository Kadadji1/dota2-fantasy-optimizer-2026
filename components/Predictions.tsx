...
const teams: TeamOdds[] = [
  ...
  { team: "Xtreme Gaming", odds: { "4-0": 3.4, "4-1": 9.2, advancing: 33, eliminated: 42, "1-4": 9.4, "0-4": 3.3 }, playoffs: 45 },
...
export default function Predictions() {
  ...
  return (
    <main className="site-shell predictions-page">
      ...
      <section className="section team-odds-section" id="team-odds">
        <div className="section-title"><div><span className="eyebrow">TEAM ODDS</span><h2>Probability of every slot</h2></div><p>Each row shows the share of model runs in which the team finished in that exact category.</p></div>
        <div className="odds-table-wrap">
          <table className="odds-table rich-odds-table">
            <thead><tr><th>Team</th><th>Rating</th>{categories.map((category)=><th key={category}>{labels[category].title}</th>)}<th>Playoffs</th></tr></thead>
            <tbody>{teams.map((team) => {
              const active = selectedCategory.get(team.team);
              return <tr key={team.team}>
                <th><span className="odds-team-cell"><TeamLogo team={team.team} size="sm" /><span><b>{team.team}</b><small>{active ? `Selected: ${labels[active].title}` : "Not selected"}</small></span></span></th>
                <td className="rating-cell">{team.rating ? <><b>{team.rating}</b><small>± {team.uncertainty}</small></> : <span className="rating-pending">—</span>}</td>
                {categories.map((category)=><td key={category} className={active === category ? "selected-odds-cell" : ""}>{formatPercent(team.odds[category])}</td>)}
                <td className="playoffs-cell"><b>{formatPercent(team.playoffs)}</b><span className="playoff-bar"><i style={{width:`${team.playoffs}%`}} /></span></td>
              </tr>;
            })}</tbody>
          </table>
        </div>
        <p className="odds-source-note">Probabilities and visible ratings were transcribed from the supplied balanced-model screenshots. Ratings not visible in those screenshots are left blank rather than inferred.</p>
      </section>
    </main>
  );
}
